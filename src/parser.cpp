...
{
    GetNextToken();
    if (nextToken_.type == TokenType::EPSILON) {
        return;
    }
...
{
    if (nextToken_.type != TokenType::FN) {
        throw "FN expected";
    }
    GetNextToken();
    if (nextToken_.type != TokenType::IDENTIFIER) {
        throw "IDENTIFIER expected";
    }
    auto functionNode =
    std::make_shared<FunctionDefinitionNode>(nextToken_.value);
    GetNextToken();
    if (nextToken_.type != TokenType::L_BRACE) {
        throw "L_BRACE expected";
    }
    GetNextToken();
    ArgumentsList(functionNode);
    if (nextToken_.type != TokenType::R_BRACE) {
        throw "R_BRACE expected";
    }
    TypeSpecifier(functionNode);
    GetNextToken();
    if (nextToken_.type != TokenType::L_CURLY_BRACE) {
        throw "L_CURLY_BRACE expected";
    }
    Expressions(functionNode);
    if (nextToken_.type != TokenType::R_CURLY_BRACE) {
...
void Parser::ArgumentTrails(std::shared_ptr<FunctionDefinitionNode> functionDefinitionNode)
{
    ...
    if (nextToken_.type != TokenType::COMMA) {
...
{
    GetNextToken();
    if (nextToken_.type != TokenType::COLON) {
...
{
    GetNextToken();
    while(nextToken_.type != TokenType::R_CURLY_BRACE) {
        Expression(node);
        GetNextToken();
    }
}
...
void Parser::FunctionReturn(std::shared_ptr<TypedNode> node)
{
    auto variableDeclarationNode = std::make_shared<FunctionReturnNode>();
    GetNextToken();
    Rvalue(variableDeclarationNode);
    node->AddChild(variableDeclarationNode);
}
...
{
    GetNextToken();
    if (nextToken_.type != TokenType::EQ) {
...
void Parser::Rvalue(std::shared_ptr<Node> node)
{
    ...
    switch (nextToken_.type) {
        ...
        default:
            throw "STRING_LITERAL or INTEGER or IDENTIFIER expected for rvalue";
    }
    node->AddChild(child);
}
...
{
    GetNextToken();
    while (nextToken_.type != TokenType::R_BRACE) {
        Rvalue(node);
